...
int *find_path(const AdjacencyMatrix *residual_capacity, int numNodes, int source, int sink, bool print_progress = false)
{
	...
	while (!to_visit.empty())
	{
		...
		if (!visited[current])
		{
			...
			{ // iterate through the current column of residual matrix, to find all neighbors
				...
				{ // if we find a neighbor that hasn't been visited
...
int ford_fulkerson(const AdjacencyMatrix *capacity, int numNodes, int source, int sink, bool print_progress = false)
{
	...
	while (next_path != NULL)
	{
		...
		while (current_node != sink)
		{ // repeat until we reach the sink
			...
			flow.set_weight(next_node, current_node, (flow.get_weight(next_node, current_node) - new_flow));
			...
			residual_capacity.set_weight(next_node, current_node, (residual_capacity.get_weight(next_node, current_node) + new_flow));
